...
export var config = {
  ...
  alwaysCompile: {
    title: '',
    description: 'Atempt to compile .styl files without a parameter line by using defaults.',
    type: 'boolean',
    default: 'true'
  },
  default: {
    ...
    properties: {
      out: {
        title: 'Out',
        description: 'Path of css file to create.',
        type: 'string',
        default: '$1.css',
        order: 1
      },
      ...
      compress: {
        title: 'Compress',
        description: 'Minify the resulting css.',
        type: 'boolean',
        default: true
      },
      sourcemap: {
        title: 'Sourcemap',
        description: 'Create a sourcemap for the file.',
        type: 'boolean',
        default: false
      }
    }
...
function getParams(source, inFile) {
  ...
  if (!paramString)
    if (atom.config.get('stylus-autocompile.alwaysCompile'))
      return {
        out: replacePlaceholders(atom.config.get('stylus-autocompile.default.out'), inFile),
        compress: atom.config.get('stylus-autocompile.default.compress'),
        sourcemap: atom.config.get('stylus-autocompile.default.sourcemap'),
        libs: atom.config.get('stylus-autocompile.default.libs')
      };
    else
      return;
